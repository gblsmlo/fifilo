...
/**
 * The one application-side id generator (Decision 014): every primary key not
 * generated by Better Auth comes from here, so a future algorithm swap is a
 * one-line change instead of a grep across the workspace. `generateId` stays
 * unbranded for observability's request ids, which are not entities.
 */
export const generateId = (): string => crypto.randomUUID()

export const generateEntityId = (): EntityId => generateId() as EntityId
