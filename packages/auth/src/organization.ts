...
export const organizationOptions = {
  // `defaultAc`'s own `newRole` is typed against `defaultStatements`'s literal
  // tuples, which TS's structural check on `OrganizationOptions.ac` (a bare
  // `AccessControl`) does not accept without this cast - a known friction
  // point reusing Better Auth's own default access control instance.
